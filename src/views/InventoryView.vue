...
<template>
  <div id="filters">
    <div class="f">
      ...
      <label style="font-weight: bold" for="">Res: </label>
      <input
        class="inputGoTo"
        type="number"
        @change="changeRows()"
        ...
        :max="items.length"
      />
      <span style="font-weight: bold">&nbsp; de {{ items.length }}</span>
    </div>
  ...
  <div class="table-conteiner">
    <div class="sb">
      <div class="div-exports">
        ...
        <div @click="exportExcel" class="excel">
          <img
            class="icon-exportar"
            src="@/assets/icon_excel.png"
            alt="Boton para descargar archivo excel"
          />
          <strong></strong>
        </div>
      ...
      <div class="div-btn">
        <button
          :class="{
            'mrg-r': selectedItems.length === 0,
            'btn-add': selectedItems.length > 0,
          }"
          @click="openModalAlta"
          type="button"
        >
      ...
      <table class="responsive-table">
        <thead>
          <tr>
            ...
            <th class="d-none">Comentarios</th>
        ...
        <tbody>
          <tr v-for="(item, index) in itemsList" :key="item.id">
            ...
            <td @click="openModalImages(item.image)" class="scale-image">
              <img
                :src="'https://api.inbeartory.site/storage/images/' + item.image"
                alt="item.image"
                class="item-image"
              />
            </td>
            ...
            <td class="d-none">{{ item.comments }}</td>
            <td class="d-none">
              <span style="font-size: 1rem" v-if="item.state === 1"
                >Activo</span
              >
              <span style="font-size: 1rem" v-else>Inactivo</span>
            </td>
            <td>
              <img
              ...
              />
              <!-- <img  @click="openModalDelete(item, index )" class="icon-actions" src="@/assets/eliminar.png" alt="Boton para eliminar">  -->
            </td>
          </tr>
        </tbody>
      </table>
  ...
  <ModalAltaItem @itemAdded="addItemToList" ref="modalAlta" />
  <ModalEditItem
    @itemEdited="editItemToList"
    @imgItemEdited="editImgItem"
    ref="modalEdit"
  />
...
<script>
import ModalAltaItem from "@/components/inventario_components/ModalAltaItem.vue";
import ModalEditItem from "@/components/inventario_components/ModalEditItem.vue";
  ...
  methods: {
    ...
    addItemToList(newItem) {
      if (this.items.length === 0) {
        ...
        if (this.isActiveFilterArea || this.isActiveFilterState) {
    ...
    editItemToList(item_edited, index) {
      const pageIndex = (this.page - 1) * this.rows + index;
      this.items[pageIndex] = item_edited;
      this.total_items = this.items.length;
    },
    editImgItem(item_edited, index) {
      const pageIndex = (this.page - 1) * this.rows + index;
      this.items[pageIndex] = item_edited;
      this.total_items = this.items.length;
    },
    changeRows() {
      if (!this.rows) {
        this.rows = 10;
        if (this.rows < this.items.length) {
          this.rows;
        } else {
    ...
    watcherRows() {
      if (this.rows < 0 || this.rows > this.items.length) {
        Swal.fire({
    ...
    openModalAlta() {
      if (this.$refs.modalAlta) {
        this.$refs.modalAlta.showModalAltaItem = true;
      }
    },
    openModalEdit(item, index) {
      if (this.$refs.modalEdit) {
        this.$refs.modalEdit.showModalEditItem = true;
        this.$refs.modalEdit.setItem(item, index);
      }
    },
    openModalDelete() {
    ...
    updateItemList() {
      ...
      if (user_role === 1) {
        this.$axios
          .get("https://api.inbeartory.site/api/v1/items", {
            headers: {
        ...
        this.$axios
          .post(
            "https://api.inbeartory.site/api/v1/user/items",
            JSON.stringify(data),
            {
              headers: {
    ...
    searchByFilters() {
      ...
      this.$axios
        .get("https://api.inbeartory.site/api/v1/items_filter", {
          params: params,
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
        ...
        .then((response) => {
          this.total_items = 0;
          this.items = response.data.Items;
          this.total_items_filters = this.items.length;
          // this.rows = this.total_items_filters;
          // this.page = 1;
          if (this.rows > this.items.length) {
            this.rows = 10;
            if (this.rows < this.items.length) {
    ...
    searchBar() {
      if (this.searchByBar === "") {
        return this.updateItemList(), this.searchByFilters();
      }
      this.$axios
    ...
    exportExcel() {
      ...
      window.location.href = `https://api.inbeartory.site/api/v1/document/inventory/excel/${user}/${area}`;
    },
    exportPDF() {
      var area = "all";
      var user = 1;
      ...
      window.location.href = `https://api.inbeartory.site/api/v1/document/inventory/pdf/${user}/${area}`;
    },
    validateRows() {
      if (this.rows <= 0) {
        this.rows = 1;
  ...
  mounted() {
    ...
    if (user_role === 1) {
      this.$axios
        ...
        .catch(() => {
          alert("No tienes áreas asignadas aún");
        });
    } else {
      this.$axios
